...
async function phasePublish(apps, cycleNum) {
  ...
  for (const app of apps) {
    ...
    try {
      ...
      const exCheck = toSlash(path.join(SKILLS_HUB, 'example', slug));
      if (fs.existsSync(exCheck)) {
        log('warn', `example/${slug} already exists, skipping`);
        continue;
      }
      ...
      const exDirNative = path.resolve(SKILLS_HUB, 'example', slug);
      ...
      // Update catalog
      const catalogPath = path.join(SKILLS_HUB, 'example', 'README.md');
      if (fs.existsSync(catalogPath)) {
        let catalog = fs.readFileSync(catalogPath, 'utf8');
        const newRow = `| [${slug}](${slug}/) | ${manifest.title} — auto-generated ${app.keyword} tool | html, css, vanilla-js | ${new Date().toISOString().split('T')[0]} |`;
        catalog = catalog.replace(/(## Adding an example)/, `${newRow}\n\n$1`);
        fs.writeFileSync(catalogPath, catalog);
      }

      // Commit + push
      await shell(`git -C "${SKILLS_HUB}" add "example/${slug}/" "example/README.md"`);
