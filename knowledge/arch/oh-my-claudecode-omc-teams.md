...
type: knowledge
category: arch
tags: [external-reference, oh-my-claudecode, omc, workflow, omc-teams]
...
## Fact
oh-my-claudecode (OMC) 는 `omc-teams` 이름의 스킬로 다음 의도를 구현한다: CLI-team runtime for claude, codex, or gemini workers in tmux panes when you need process-based parallel execution. 원본 위치: `skills/omc-teams/SKILL.md` in `https://github.com/Yeachan-Heo/oh-my-claudecode` (commit `09ffccc`).

## Context / Why
- OMC는 널리 사용되는 멀티에이전트 오케스트레이션 레이어이며, 해당 도메인에서 **성숙한 한 가지 접근법**의 레퍼런스다.
- 우리 프로젝트의 같은 의도의 스킬을 설계·개선할 때 비교 기준점이 된다.
- 원본 SKILL.md 전문은 로컬 `~/.claude/skills-hub/knowledge/reference/oh-my-claudecode-omc-teams.md` 에 보존돼 있어 언제든 상세 확인 가능하다.
